...
//실시간 투표율(임시)
                                ...
                                //reload
                                var _reloagetDataTimer = null;
                                $(document).ready(function(){
                                    reloagetData();
                                });
                                function reloagetData(){    
                                    clearTimeout(_reloagetDataTimer); // clear timer
                                    tupyoCounter();                                 
                                    _reloagetDataTimer  = setTimeout("reloagetData()", 30000 );
                                }
                                //투표 카운트
                                function tupyoCounter(){    
                                        var actionUrl = "https://cors-anywhere.herokuapp.com/http://news.kbs.co.kr/special/election2020/getLiveData.do";
                                        $.ajax({
                                            url         : actionUrl,
                                            type        : "get",
                                            dataType    : 'json',
                                            success     : function( data, status ) {
                                                    ...
                                                    var tupyoTime = data.TUPYO.basetime;
                                                    tupyoTime = tupyoTime.substring(8,10)+":"+tupyoTime.substring(10,12);
                                                    
                                                    var appendHtml = '<a href="https://m.news.naver.com/election/nation2020/result/index.nhn?p=party" title="투개표 페이지 이동" target="_blank">';
                                                    ...
                                                    $(".election2020-rate-box").append(appendHtml);
                                                    countAnimate();                         
                                            }
                                        });
                                }
                                function countAnimate(){
                                    $('.rate > .num').each(function () {                        
